...
//        return cardNo.substring(length - afterLength, length);
    ...
    public static Number number(Object value){
        return number(value,0,Long.MAX_VALUE);
    }

    public static Number number(Object value, int def, long max){
        if (value == null){
            return def;
        }
        return toNumber(value.toString(), def, max);
    }

    public static Number toNumber(String str){
        return toNumber(str, 0, Long.MAX_VALUE);
    }

    public static Number toNumber(String str, int def, long max) {
        ...
        try{
            if (str.indexOf(".") > -1) {
                ...
                return doubleValue > max ? def : doubleValue;
            } else {
                Long longValue = Long.parseLong(str);
                return longValue > max ? def : longValue;
            }
        }catch (Exception ex){
        }
        return def;
    ...
    public static String toUnderlineName(String s, boolean isUpperCase){
        ...
        if (Character.isUpperCase(s.charAt(0)) && isUpperCase){
            return s;
        }
