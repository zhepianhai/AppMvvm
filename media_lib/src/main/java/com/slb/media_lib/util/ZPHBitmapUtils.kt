...
object ZPHBitmapUtils {
    ...
    fun savePic(
        ...
        thread {
            try {
                ...
                var picFile = File(path, "$timeStamp.jpg")
    ...
    fun saveYuv420_422(
        ...
        thread {
            ...
            //回传的是YUV422
            if (y.size / u.size == 2) {
                ZPHImageUtil.yuv422ToYuv420sp(y, u, v, nv21, stride, previewSize.height)
            } else if (y.size / u.size == 4) {
                ZPHImageUtil.yuv420ToYuv420sp(y, u, v, nv21, stride, previewSize.height)
            }
...
//            }
            ...
            val temp = System.currentTimeMillis()
            var path =
                Environment.getExternalStorageDirectory().path + File.separator + "zphMedia/YUV"
            val timeStamp = SimpleDateFormat("yyyyMMdd_HHmmss").format(Date())
