...
//        mText = ""
    ...
    fun setColorType(type: Int) {
        ...
        when (type) {
            ...
            TYPE_AUDIO -> {
                mBtnBackground = ContextCompat.getColor(context, R.color.colorYel)
                mBtnBackGroupD = ContextCompat.getColor(context, R.color.colorYel_d)
        ...
        if(canTouch) {
            when (event.action) {
                MotionEvent.ACTION_DOWN -> {
                    mOnRecordListener?.onRecordStar()
                    mLastX = event.rawX
                    mLastY = event.rawY
                    mTouchDown = true
                    valueAnimator.start()
